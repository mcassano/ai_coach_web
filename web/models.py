...
class Exercise(models.Model):
    ...
    PLANK = 'Plank'
    STANDINGCROSS = 'Standing-Cross'

    EXERCISE_CHOICES = [
        (PUSHUP, PUSHUP),
        (FLAPPINGCROSS, FLAPPINGCROSS),
        (PLANK, PLANK),
        (STANDINGCROSS, STANDINGCROSS)
    ]
...
class ExerciseSet(models.Model):
    ...
    num_target_reps = models.IntegerField()
    ...
    duration_target_seconds = models.IntegerField()

    def __str__(self):
        return (f'{self.performed_by} did '
                f'{self.exercise_performed.name} '
                f'{self.num_reps} ({self.num_target_reps}) time(s) for '
                f'{self.duration_seconds} ({self.duration_target_seconds}) seconds on '
